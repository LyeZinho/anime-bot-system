...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Se quantidade for 0, deposita tudo
    if (amount === 0) {
    ...
    const result = await userService.bankDeposit(interaction.user.id, amount);
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription([
        `Você depositou **${amount.toLocaleString()}** moedas no banco!`,
        '',
